...
hip = sigmoid(z_3);
cost1 = -y_label'.*log(hip);
cost2 = (1-y_label)'.*log(1-hip);
cost_h = sum(cost1 - cost2);

Theta1_non_zero = Theta1;
Theta1_non_zero(:,1) = [];

Theta2_non_zero = Theta2;
Theta2_non_zero(:,1) = [];

cost_reg = (sum(sum(Theta1_non_zero.^2))+sum(sum(Theta2_non_zero.^2)))*lambda/(2*m);
J = sum(cost_h)/m + cost_reg;
